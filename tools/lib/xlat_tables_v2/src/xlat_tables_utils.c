...
void xlat_mmap_print(const mmap_region_t *mmap)
{
    ...
    while (mm->size != 0U) {
                LOG(DBG, " VA:0x%lx  PA:0x%lx size:0x%lx attr:0x%x granularity:0x%lx\n",
                  mm->base_va, mm->base_pa, mm->size, mm->attr, mm->granularity);
        ++mm;
    };
    LOG(DBG, "\n");
}
...
static void xlat_desc_print(const xlat_ctx_t *ctx, uint64_t desc)
{
    ...
    if ((xlat_regime == EL3_REGIME) || (xlat_regime == EL2_REGIME)) {
        ...
        LOG(DBG, ((desc & LOWER_ATTRS(AP_RO)) != 0ULL) ? "-RO" : "-RW");
        LOG(DBG, ((desc & UPPER_ATTRS(XN)) != 0ULL) ? "-XN" : "-EXEC");
    } else {
        ...
        LOG(DBG, ((desc & UPPER_ATTRS(PXN)) != 0ULL) ? "-XN" : "-EXEC");
        /*
        ...
        if ((desc & DESC_MASK) == INVALID_DESC) {
            ...
            if (((desc & DESC_MASK) == TABLE_DESC) &&
               ...
               LOG(DBG, "%sVA:0x%lx size:0x%lx \n",
                ...
                LOG(DBG, "%sVA:0x%lx PA:0x%lx size:0x%lx ",
...
#endif
    LOG(DBG, "  Used %d sub-tables out of %d\n", used_page_tables, (uint64_t)ctx->tables_num);
    LOG(DBG, "(spare: %d)\n", (uint64_t)ctx->tables_num - used_page_tables);
