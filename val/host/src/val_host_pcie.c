...
__attribute__((aligned (PAGE_SIZE * 2))) pcie_device_bdf_table
                  g_pcie_bdf_table1[PCIE_DEVICE_BDF_TABLE_SZ];
...
uint32_t val_pcie_read_cfg(uint32_t bdf, uint32_t offset, uint32_t *data)
{
    ...
    if ((bus >= PCIE_MAX_BUS) || (dev >= PCIE_MAX_DEV) || (func >= PCIE_MAX_FUNC)) {
      LOG(ERROR, "       Invalid Bus/Dev/Func  %x", bdf);
      return PCIE_NO_MAPPING;
    }

    if (g_pcie_info_table == NULL) {
        LOG(ERROR, "       Read_PCIe_CFG: PCIE info table is not created");
        return PCIE_NO_MAPPING;
    }
    ...
    if (ecam_base == 0) {
        LOG(ERROR, "       Read PCIe_CFG: ECAM Base is zero for bdf %x", bdf);
        return PCIE_NO_MAPPING;
    }
    ...
    *data = pal_mmio_read32(ecam_base + cfg_addr + offset);
    return 0;
...
val_pcie_write_cfg(uint32_t bdf, uint32_t offset, uint32_t data)
{
    ...
    if ((bus >= PCIE_MAX_BUS) || (dev >= PCIE_MAX_DEV) || (func >= PCIE_MAX_FUNC)) {
      LOG(ERROR, "Invalid Bus/Dev/Func  %x\n", bdf);
      return;
    }

    if (g_pcie_info_table == NULL) {
        LOG(ERROR, "       Write PCIe_CFG: PCIE info table is not created");
        return;
    }
    ...
    if (ecam_base == 0) {
        LOG(ERROR, "       Read PCIe_CFG: ECAM Base is zero ");
        return;
    }
    ...
    pal_mmio_write32(ecam_base + cfg_addr + offset, data);
}
...
/**
    ...
    @return  success/failure
**/
uint32_t val_pcie_bar_mem_read(uint32_t bdf, uint64_t offset, uint32_t *data)
{
...
uint64_t val_pcie_get_bdf_config_addr(uint32_t bdf)
{
    ...
    if (g_pcie_info_table == NULL) {
        LOG(ERROR, "       PCIe_CFG: PCIE info table is not created");
        return 0;
    }
...
void val_pcie_print_device_info(void)
{
    ...
    {
      LOG(DBG, "  BDF Table: No RCiEP or iEP found\n");
      return;
    }
    ...
    LOG(TEST, "PCIE_INFO: Number of RCiEP           : %d\n", num_rciep);
    LOG(TEST, "PCIE_INFO: Number of RCEC            : %d\n", num_rcec);
    LOG(TEST, "PCIE_INFO: Number of EP              : %d\n", num_ep);
    LOG(TEST, "PCIE_INFO: Number of RP              : %d\n", num_rp);
    LOG(TEST, "PCIE_INFO: Number of iEP_EP          : %d\n", num_iep);
    LOG(TEST, "PCIE_INFO: Number of iEP_RP          : %d\n", num_irp);
    LOG(TEST, "PCIE_INFO: Number of UP of switch    : %d\n", num_up);
    LOG(TEST, "PCIE_INFO: Number of DP of switch    : %d\n", num_dp);
    LOG(TEST, "PCIE_INFO: Number of PCI/PCIe Bridge : %d\n", num_pci_pcie);
    LOG(TEST, "PCIE_INFO: Number of PCIe/PCI Bridge : %d\n", num_pcie_pci);
    ...
    while (ecam_index < val_pcie_get_info(PCIE_INFO_NUM_ECAM, 0))
    {
        ...
        while (tbl_index < bdf_tbl_ptr->num_entries)
        {
                ...
                && (bus_num <= ecam_end_bus))
            {
                ...
                LOG(TEST, "  BDF: 0x%x\n", bdf);
                LOG(TEST, "  Seg: 0x%x, ", seg_num);
                LOG(TEST, "Bus: 0x%x, ", bus_num);
                LOG(TEST, "Dev: 0x%x, ", dev_num);
                LOG(TEST, "Func: 0x%x, ", func_num);
                LOG(TEST, "Dev ID: 0x%x, ", device_id);
                LOG(TEST, "Vendor ID: 0x%x\n", vendor_id,  0);
            }
        }
        if (bdf_counter == 0)
            LOG(INFO, "  No BDF devices in ECAM region index %d\n", ecam_index);
...
void val_pcie_create_info_table(uint64_t *pcie_info_table)
{
    ...
    if (pcie_info_table == NULL) {
        LOG(ERROR, "Input for Create Info table cannot be NULL\n");
        return;
    }
    LOG(INFO, " Creating PCIe INFO table\n");

    g_pcie_info_table = (PCIE_INFO_TABLE *)pcie_info_table;
    g_pcie_bdf_table = (pcie_device_bdf_table *)g_pcie_bdf_table1;

    pal_pcie_create_info_table(g_pcie_info_table);

    num_ecam = (uint32_t)val_pcie_get_info(PCIE_INFO_NUM_ECAM, 0);
    LOG(TEST, "PCIE_INFO: Number of ECAM regions    :    %ld\n",
                                        val_pcie_get_info(PCIE_INFO_NUM_ECAM, 0), 0);
    if (num_ecam == 0)
        return;
    ...
    if (val_pcie_create_device_bdf_table()) {
        LOG(ERROR, "Create Bdf table failed.\n");
        return;
    }
...
static uint32_t val_pcie_populate_device_rootport(void)
{
    ...
    for (tbl_index = 0; tbl_index < bdf_tbl_ptr->num_entries; tbl_index++)
    {
        ...
        if (status == 0) {
            LOG(TEST, "Dev bdf: 0x%x RP bdf: 0x%x\n", bdf,
                 rp_bdf);
           } else {
            LOG(TEST, "Dev bdf: 0x%x RP bdf: none\n", bdf);
        }
...
uint32_t val_pcie_create_device_bdf_table(void)
{
    ...
    if (!g_pcie_bdf_table)
    {
        LOG(ERROR, "       PCIe BDF table memory allocation failed          ");
        return 1;
    }
            ...
            for (dev_index = 0; dev_index < PCIE_MAX_DEV; dev_index++)
            {
                for (func_index = 0; func_index < PCIE_MAX_FUNC; func_index++)
                {
                    ...
                    if (val_pcie_read_cfg(bdf, TYPE01_VIDR, &reg_value) == PCIE_NO_MAPPING)
                    {
                        ...
                        LOG(ERROR, "       BDF 0x%x mapping issue", bdf);
                        return 1;
                    }
                    ...
                    if (reg_value != PCIE_UNKNOWN_RESPONSE)
                    {
                        ...
                        if (val_pcie_find_capability(bdf, PCIE_CAP,
...
addr_t val_pcie_get_ecam_base(uint32_t bdf)
{
    ...
    seg_num = (uint16_t)PCIE_EXTRACT_BDF_SEG(bdf);
...
uint64_t val_pcie_get_info(PCIE_INFO_e type, uint32_t index)
{
    ...
    if (g_pcie_info_table == NULL) {
        LOG(ERROR, "GET_PCIe_INFO: PCIE info table is not created\n");
        return 0;
    }

    if (index >= g_pcie_info_table->num_entries) {
        if (g_pcie_info_table->num_entries != 0)
            LOG(ERROR, "Invalid index %d > num of entries\n", index);
        return 0;
    }
    ...
    default:
        LOG(ERROR, "This PCIE info option not supported %d\n", type);
        break;
    }
...
uint32_t val_pcie_increment_bdf(uint32_t bdf)
{
    ...
    ecam_cnt = (int32_t)val_pcie_get_info(PCIE_INFO_NUM_ECAM, 0);
    while (ecam_cnt--) {
        if (seg == val_pcie_get_info(PCIE_INFO_SEGMENT, (uint32_t)ecam_cnt)) {
            ecam_index = (uint32_t)ecam_cnt;
            break;
        }
    }
...
void val_pcie_disable_bme(uint32_t bdf)
{
    ...
    dis_mask = ~((uint32_t)1 << CR_BME_SHIFT);
...
void val_pcie_disable_msa(uint32_t bdf)
{
    ...
    val_pcie_read_cfg(bdf, TYPE01_CR, &reg_value);
    dis_mask = ~((uint32_t)1 << CR_MSE_SHIFT);
    val_pcie_write_cfg(bdf, TYPE01_CR, reg_value & dis_mask);
...
void val_pcie_disable_eru(uint32_t bdf)
{
    ...
    */
    val_pcie_read_cfg(bdf, TYPE01_CR, &reg_value);
    dis_mask = ~((uint32_t)1 << CR_SERRE_SHIFT);
    val_pcie_write_cfg(bdf, TYPE01_CR, reg_value & dis_mask);
    ...
    * detected by the Function.
    */
    dis_mask = ~((uint32_t)1 << DCTLR_CERE_SHIFT |
                (uint32_t)1 << DCTLR_NFERE_SHIFT |
                (uint32_t)1 << DCTLR_FERE_SHIFT |
                (uint32_t)1 << DCTLR_URRE_SHIFT);
    val_pcie_write_cfg(bdf, pciecs_base + DCTLR_OFFSET, reg_value & dis_mask);
}
    ...
    */
    ...
    switch (bf_entry->reg_type)
    {
        ...
        case PCIE_ECAP:
            ...
            break;
        default:
            LOG(ERROR, "       Invalid reg_type : 0x%x  ", bf_entry->reg_type);
            return 1;
    }

    if (status != PCIE_SUCCESS)
    {
        LOG(ERROR, "       PCIe Capability 0x%x", id);
        LOG(ERROR, " not found for BDF 0x%x\n", bdf);
        return status;
    }
    ...
    {
        LOG(ERROR, "       BDF 0x%x : ", bdf);
        LOG(ERROR, bf_entry->err_str1);
        LOG(ERROR, ": 0x%x", bf_value);
        LOG(ERROR, " instead of 0x%x\n", bf_entry->cfg_value);
        if (!val_strcmp(bf_entry->err_str1, "WARNING"))
            return 0;
        return 1;
    }
    ...
    switch (bf_entry->attr)
    {
        ...
        case RSVDP_RO:
            ...
            val_pcie_write_cfg(bdf, cap_base + reg_offset, reg_overwrite_value);
            val_pcie_read_cfg(bdf, cap_base + reg_offset, &reg_overwrite_value);
            reg_overwrite_value = (reg_overwrite_value >>
                      REG_SHIFT(alignment_byte_cnt, bf_entry->start)) &
                      REG_MASK(bf_entry->end, bf_entry->start);
        ...
        case STICKY_RW:
            ...
            break;
        default:
            LOG(ERROR, "       Invalid Attribute : 0x%x  ", bf_entry->attr);
            return 1;
    }
    ...
    {
        LOG(ERROR, "       BDF 0x%x : ", bdf);
        LOG(ERROR, bf_entry->err_str2);
        LOG(ERROR, ": 0x%x", reg_overwrite_value);
        LOG(ERROR, " instead of 0x%x\n", reg_value);
        if (!val_strcmp(bf_entry->err_str2, "WARNING"))
            return 0;
        return 1;
    }
...
uint32_t val_pcie_get_rootport(uint32_t bdf, uint32_t *rp_bdf)
{
    ...
    /* Return failure */
    LOG(ERROR, "       PCIe Hierarchy fail: RP of bdf 0x%x not found", bdf);
    *rp_bdf = 0;
    return 1;
...
uint32_t val_pcie_link_cap_support(uint32_t bdf)
{
    ...
    if (reg_value != 0) {
      LOG(ERROR, "       Link Capabilities reg check failed");
      return 1;
    }
    ...
    if (reg_value != 0) {
      LOG(ERROR, "       Link Capabilities control and status check failed");
      return 1;
    }
    ...
    if (reg_value != 0) {
      LOG(ERROR, "       Link Capabilities 2 reg check failed");
      return 1;
  }
    ...
    if (reg_value != 0) {
      LOG(ERROR, "       Link Capabilities 2 control and status check failed");
      return 1;
    }
...
/**
  @brief  Get PCIe BDF table pointer
  @param  void
  @return number  of entries in bdf table
**/
uint32_t val_pcie_get_num_bdf(void)
{
    pcie_device_bdf_table *bdf_tbl_ptr;
    ...
    if (bdf_tbl_ptr->num_entries == 0)
    {
        LOG(ERROR, "No BDFs discovered");
        return VAL_ERROR;
    }

    return bdf_tbl_ptr->num_entries;
}

/**
  @brief  Checks if DOE Capabilities is supported
  @param  num_bdf   - last index in PCIe bdf table pointer
  @param  bdf       - Pointer to store BDF
  @return Success/failer
**/
uint64_t val_pcie_find_doe_capability(uint32_t *num_bdf, uint32_t *bdf, uint32_t *doe_cap_base)
{
    pcie_device_bdf_table *bdf_tbl_ptr;
    uint32_t pcie_doe_cap_base = 0, status = 0, pcie_bdf;
    uint32_t dp_type;
    ...
    total_entries = bdf_tbl_ptr->num_entries;

    /* Traverse forward across calls, while consuming (*num_bdf) */
    while (*num_bdf != 0)
    {
        uint32_t idx = total_entries - *num_bdf;
        (*num_bdf)--;

        pcie_bdf = bdf_tbl_ptr->device[idx].bdf;

        /*
         * Skip VF in multi function device as it can't be treated as
         * PDEV
         */
        if (PCIE_EXTRACT_BDF_FUNC(pcie_bdf) != 0) {
            continue;
        }

        /* Get device/port type */
        dp_type = val_pcie_device_port_type(pcie_bdf);

        /* Consider only EP and RCiEP */
        if ((dp_type != EP) && (dp_type != RCiEP)) {
            continue;
        }

        /* If EP, ensure RP exists */
        if ((dp_type == EP) && (bdf_tbl_ptr->device[idx].rp_bdf == 0)) {
            LOG(DBG, "No RP found for Device %x:%x.%x\n",
                PCIE_EXTRACT_BDF_BUS(pcie_bdf),
                PCIE_EXTRACT_BDF_DEV(pcie_bdf),
                PCIE_EXTRACT_BDF_FUNC(pcie_bdf));
            continue;
        }

        /* Check DOE capability */
        status = val_pcie_find_capability(pcie_bdf, PCIE_ECAP, DOE_CAP_ID, &pcie_doe_cap_base);
        if (status == PCIE_CAP_NOT_FOUND)
        {
            LOG(DBG, "\n PCIe DOE capability not found %x", pcie_bdf);
            continue;
        }

        LOG(DBG, "\n PCIe DOE capability found %x", pcie_bdf);
        *bdf = pcie_bdf;
        *doe_cap_base = pcie_doe_cap_base;
        return VAL_SUCCESS;
    }

    LOG(ERROR, " No PCIe DOE capability supported device found");
    return VAL_ERROR;
}
...
static uint32_t pci_bar_size_helper(uint32_t bdf, uint32_t offset)
{
    ...
    val_pcie_read_cfg(bdf, offset, &bar);
    val_pcie_write_cfg(bdf, offset, 0xFFFFFFFF);
    val_pcie_read_cfg(bdf, offset, &val);
    val_pcie_write_cfg(bdf, offset, bar);

    return val;
}

uint32_t val_pcie_get_bar(uint32_t bdf, uint32_t offset, uint64_t *addr, uint64_t *size)
{
    ...
    val_pcie_read_cfg(bdf, offset, &bar_0);

    size_0 &= BAR_MASK;
    offset += 4;

    if (BAR_REG(bar_0) == BAR_64_BIT) {
        uint32_t bar_4, size_4;

        val_pcie_read_cfg(bdf, offset, &bar_4);
        size_4 = pci_bar_size_helper(bdf, offset);
...
void val_pcie_mem_enable(uint32_t bdf)
{
    uint32_t com_reg_value;

    /* Enable memory access, Bus master enable, I/O access and SERR# Enable */
    val_pcie_read_cfg(bdf, COMMAND_REG_OFFSET, &com_reg_value);
    val_pcie_write_cfg(bdf, COMMAND_REG_OFFSET, (com_reg_value | REG_ACC_DATA | SERR_ENABLE));
}