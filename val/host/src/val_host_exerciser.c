...
void val_exerciser_create_info_table(void)
{
    ...
    while (num_bdf-- != 0)
    {
         ...
         if ((Bdf))
         {
              ...
              LOG(DBG, "      exerciser Bdf %x\n", Bdf);
         }
    }
    LOG(TEST, "      PCIE_INFO: Number of exerciser cards : %d\n",
...
uint32_t val_get_exerciser_err_info(EXERCISER_ERROR_CODE type)
{
    ...
    default:
         LOG(ERROR, "   Invalid error offset ");
         return 0;
    }
}
...
uint32_t val_exerciser_init(uint32_t instance)
{
    ...
    if (!g_exerciser_info_table.e_info[instance].initialized)
    {
         ...
         if (pal_exerciser_get_state(&state, Bdf) || (state != EXERCISER_ON)) {
              LOG(ERROR, "   Exerciser Bdf %lx not ready", Bdf);
              return 1;
         }
          ...
          pal_mmio_write32((Ecam + cfg_addr + COMMAND_REG_OFFSET),
                    (pal_mmio_read32((Ecam + cfg_addr) + COMMAND_REG_OFFSET) | BUS_MEM_EN_MASK));
