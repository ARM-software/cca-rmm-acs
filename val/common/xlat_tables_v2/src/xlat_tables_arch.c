...
bool is_mmu_enabled_ctx(const xlat_ctx_t *ctx)
{
	if (ctx->xlat_regime == EL1_EL0_REGIME) {
		...
		return (val_sctlr_read(xlat_arch_current_el()) & SCTLR_M_BIT) != 0U;
...
bool is_dcache_enabled(void)
{
	...
	if (el == 1U) {
		return (val_sctlr_read(el) & SCTLR_C_BIT) != 0U;
	} else if (el == 2U) {
		return (val_sctlr_read(el) & SCTLR_C_BIT) != 0U;
	} else {
		return (val_sctlr_read(el) & SCTLR_C_BIT) != 0U;
...
{
	unsigned int el = (unsigned int)val_read_current_el();
		   ...
		   uintptr_t max_va, int xlat_regime)
{
	...
	mair = MAIR_ATTR_SET(ATTR_DEV, ATTR_DEV_INDEX);
