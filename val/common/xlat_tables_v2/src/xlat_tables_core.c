		   ...
		   unsigned long long addr_pa, unsigned int level)
{
	...
	desc |= (level == XLAT_TABLE_LEVEL_MAX) ? PAGE_DESC : BLOCK_DESC;
        ...
        /*
	 * Clear Access flag if MT_AF_CLEAR is set otherwise Always set the access flag, as this
	 * library assumes access flag faults aren't managed.
	 */
        if ((attr & MT_AF_CLEAR) == 0U)
	...
	if (mem_type == MT_DEVICE) {
		desc |= LOWER_ATTRS(ATTR_DEV_INDEX | OSH);
		/*
			    ...
			    int xlat_regime, int *mapped_regions)
{
	...
	ctx->base_table_entries = (unsigned int)GET_NUM_BASE_LEVEL_ENTRIES(va_space_size);
