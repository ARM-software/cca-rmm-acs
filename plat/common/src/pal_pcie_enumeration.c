...
uint32_t g_vendor_id, *device_id;

uint32_t pcie_index = 0, enumerate = 1;
/*64-bit address initialisation*/
uint64_t g_bar64_p_start  = PLATFORM_OVERRIDE_PCIE_BAR64_VALUE;
uint64_t g_rp_bar64_value = PLATFORM_OVERRIDE_RP_BAR64_VALUE;
uint64_t g_bar64_p_max;
uint32_t g_64_bus, g_bar64_size;

/*32-bit address initialisation*/
uint32_t g_bar32_np_start = PLATFORM_OVERRIDE_PCIE_BAR32NP_VALUE;
uint32_t g_bar32_p_start  = PLATFORM_OVERRIDE_PCIE_BAR32P_VALUE;
uint32_t g_rp_bar32_value = PLATOFRM_OVERRIDE_RP_BAR32_VALUE;
...
**/
uint32_t pal_pci_cfg_read(uint32_t bus, uint32_t dev, uint32_t func, uint32_t offset,
                                                                     uint32_t *value)
{
    ...
    while (offset <= TYPE1_BAR_MAX_OFF)
    {
        ...
        if (BAR_REG(bar_reg_value) == BAR_64_BIT)
        {
            ...
            **/
            ...
            pal_pci_cfg_write(bus, dev, func, offset, (uint32_t)g_rp_bar64_value);
            pal_pci_cfg_write(bus, dev, func, offset + 4, (uint32_t)(g_rp_bar64_value >> 32));
            offset = offset + 8;
        }
      ...
      if (BAR_TYPE(bar_reg_value) == BAR_MEM_SPACE)
      {
          if (BAR_REG(bar_reg_value) == BAR_64_BIT)
          {
              ...
              **/
              ...
              pal_pci_cfg_write(bus, dev, func, offset, (uint32_t)g_bar64_p_start);
              pal_pci_cfg_write(bus, dev, func, offset + 4, (uint32_t)(g_bar64_p_start >> 32));
              ...
              **/
              if ((p_bar_size == 0) && ((g_p_bus == bus)))
              {
                  if (g_p_bar_size < bar_size)
                      g_bar32_p_start = g_bar32_p_start + (uint32_t)bar_size;
                  else
          ...
          **/
          ...
          if ((np_bar_size == 0) && ((g_np_bus == bus)))
          {
              if (g_np_bar_size < bar_size)
                  g_bar32_np_start = g_bar32_np_start + (uint32_t)bar_size;
              else
    ...
    for (dev = 0; dev < PCIE_MAX_DEV; dev++)
    {
      for (func = 0; func < PCIE_MAX_FUNC; func++)
      {
          ...
          if (PCIE_HEADER_TYPE(header_value) == TYPE1_HEADER)
          {
              ...
              pal_pci_cfg_write(bus, dev, func, BUS_NUM_REG_OFFSET,
              ...
              /*Obtain the start memory base address & the final memory base address of 32 bit BAR*/
              bar32_p_limit = g_bar32_p_max;
              bar32_np_limit = g_bar32_np_max;
              get_resource_base_32(bus, dev, func, bar32_p_base, bar32_np_base, bar32_p_limit,
                                                                              bar32_np_limit);

              /*Obtain the start memory base address & the final memory base address of 64 bit BAR*/
...
void *pal_pci_bdf_to_dev(uint32_t bdf)
{
    ...
    pal_pci_cfg_read(bus, dev, func, 0, &g_vendor_id);
    g_vendor_id = g_vendor_id >> DEVICE_ID_OFFSET;
    device_id = &g_vendor_id;
