                                                                      ...
                                                                      uint32_t *Offset)
{
    ...
    if (Value == 1) {
        ...
        NxtPtr = ((pal_mmio_read32(Ecam + CAP_PTR_OFFSET +
                   pal_exerciser_get_pcie_config_offset(Bdf))) & CAP_PTR_MASK);
    }

    while (NxtPtr != 0) {
      Data = pal_mmio_read32(Ecam + pal_exerciser_get_pcie_config_offset(Bdf) + NxtPtr);
      TempId = Data & IdMask;
      if (TempId == ID) {
          *Offset = (uint32_t)NxtPtr;
          return 0;
      }
