...
uint32_t pal_is_bdf_exerciser(uint32_t bdf)
{
    ...
    */
    ...
    vendor_dev_id = pal_mmio_read32(Ecam + pal_exerciser_get_pcie_config_offset(bdf));
                                 ...
                                 uint64_t Value2, uint32_t Bdf)
{
    ...
    */
    ...
    case DMA_ATTRIBUTES:
        /* wrting into the DMA Control Register 2 */
        pal_mmio_write32(Base + DMA_BUS_ADDR, (uint32_t)Value1);
        /* writing into the DMA Control Register 3 */
        pal_mmio_write32(Base + DMA_LEN, (uint32_t)Value2);
        return 0;
    ...
    case PASID_ATTRIBUTES:
        Data = pal_mmio_read32(Base + DMACTL1);
        Data &= (uint32_t)~(PASID_LEN_MASK << PASID_LEN_SHIFT);
        Data |= (uint32_t)((Value1 - 16) & PASID_LEN_MASK) << PASID_LEN_SHIFT;
        pal_mmio_write32(Base + DMACTL1, Data);
        return 0;
    ...
    case CFG_TXN_ATTRIBUTES:
        ...
        case TXN_REQ_ID:
            ...
            Data = (uint32_t)((Value2 & RID_VALUE_MASK) | RID_VALID_MASK);
            pal_mmio_write32(Base + RID_CTL_REG, Data);
            return 0;
        case TXN_REQ_ID_VALID:
            switch (Value2) {
            case RID_VALID:
                Data = pal_mmio_read32(Base + RID_CTL_REG);
                Data |= RID_VALID_MASK;
                pal_mmio_write32(Base + RID_CTL_REG, Data);
                return 0;
            case RID_NOT_VALID:
                pal_mmio_write32(Base + RID_CTL_REG, 0);
                return 0;
            }
            return 0;
        case TXN_ADDR_TYPE:
            ...
            case AT_UNTRANSLATED:
                Data = 0x1;
                pal_mmio_write32(Base + DMACTL1, pal_mmio_read32(Base + DMACTL1) | (Data << 10));
                break;
            case AT_TRANSLATED:
                Data = 0x2;
                pal_mmio_write32(Base + DMACTL1, pal_mmio_read32(Base + DMACTL1) | (Data << 10));
    ...
    case ERROR_INJECT_TYPE:
        ...
        Data = pal_mmio_read32(Ecam + CapabilityOffset +
                               pal_exerciser_get_pcie_config_offset(Bdf) + DVSEC_CTRL);
        Data = (uint32_t)(((Value1 << ERR_CODE_SHIFT) | (Value2 << FATAL_SHIFT)));
        pal_mmio_write32(Ecam + CapabilityOffset + DVSEC_CTRL +
...
uint32_t pal_exerciser_start_dma_direction(uint64_t Base, EXERCISER_DMA_ATTR Direction)
{
    ...
    if (Direction == EDMA_TO_DEVICE) {
        ...
        pal_mmio_write32(Base + DMACTL1, (pal_mmio_read32(Base + DMACTL1) & Mask));
    }
    ...
    /* Triggering the DMA */
    pal_mmio_write32(Base + DMACTL1, (pal_mmio_read32(Base + DMACTL1) | MASK_BIT));

    /* Reading the Status of the DMA */

    Status = (pal_mmio_read32(Base + DMASTATUS) & ((MASK_BIT << 1) | MASK_BIT));
    return Status;
}
                                 ...
                                 uint64_t *Value2, uint32_t Bdf)
{
    ...
    */
    ...
    case SNOOP_ATTRIBUTES:
        return 0;
    case LEGACY_IRQ:
        *Value1 = pal_mmio_read32(Base + INTXCTL);
        return pal_mmio_read32(Base + INTXCTL) | MASK_BIT;
    case DMA_ATTRIBUTES:
        /* Reading the data from DMA Control Register 2 */
        *Value1 = pal_mmio_read32(Base + DMA_BUS_ADDR);
        /* Reading the data from DMA Control Register 3 */
        *Value2 = pal_mmio_read32(Base + DMA_LEN);
        Temp = pal_mmio_read32(Base + DMASTATUS);
        Status = Temp & MASK_BIT; /* returning the DMA status */
        return Status;
    case P2P_ATTRIBUTES:
        return 0;
    case PASID_ATTRIBUTES:
        *Value1 = ((pal_mmio_read32(Base + DMACTL1) >> PASID_LEN_SHIFT) & PASID_LEN_MASK) + 16;
        return 0;
    case MSIX_ATTRIBUTES:
        *Value1 = pal_mmio_read32(Base + MSICTL);
        return pal_mmio_read32(Base + MSICTL) | MASK_BIT;
    case ATS_RES_ATTRIBUTES:
        *Value1 = pal_mmio_read32(Base + ATS_ADDR);
        return 0;
    case CFG_TXN_ATTRIBUTES:
    ...
    case DATA_ATTRIBUTES:
        ...
        tx_attr = pal_mmio_read32(Base + TXN_TRACE);
        ...
        /* Obtain all the recorded information of the packet in the format.
            ...
            |________________________________|
        */
        addr_low = pal_mmio_read32(Base + TXN_TRACE);
        addr_high = pal_mmio_read32(Base + TXN_TRACE);
        data_low = pal_mmio_read32(Base + TXN_TRACE);
        data_high = pal_mmio_read32(Base + TXN_TRACE);
    ...
    */
    ...
    case START_DMA:
        ...
        case EDMA_FROM_DEVICE:
            /* DMA from Device */
            return pal_exerciser_start_dma_direction(Base, EDMA_FROM_DEVICE);
        case EDMA_TO_DEVICE:
            /* DMA to Device */
            return pal_exerciser_start_dma_direction(Base, EDMA_TO_DEVICE);
        default:
            return 1;
        }
    ...
    case GENERATE_MSI:
        ...
        pal_mmio_write32(Base + MSICTL,
                         (pal_mmio_read32(Base + MSICTL) | (uint32_t)(MSI_GENERATION_MASK) |
                          (uint32_t)(Param)));
        return 0;

    case GENERATE_L_INTR:
        pal_mmio_write32(Base + INTXCTL,
                         (pal_mmio_read32(Base + INTXCTL) | MASK_BIT));
    ...
    case CLEAR_INTR:
        pal_mmio_write32(Base + INTXCTL,
                         (pal_mmio_read32(Base + INTXCTL) & CLR_INTR_MASK));
        return 0;
    ...
    case PASID_TLP_START:
        ...
        pal_mmio_write32(Base + PASID_VAL, data);

        if (!pal_exerciser_find_pcie_capability(PASID, Bdf, PCIE, &CapabilityOffset)) {
            pal_mmio_write32(Ecam + pal_exerciser_get_pcie_config_offset(Bdf) +
                                 CapabilityOffset + PCIE_CAP_CTRL_OFFSET,
                             (pal_mmio_read32(Ecam + pal_exerciser_get_pcie_config_offset(Bdf) +
                                               CapabilityOffset + PCIE_CAP_CTRL_OFFSET)) |
                                 PCIE_CAP_EN_MASK);
            return 0;
        }
    ...
    case PASID_TLP_STOP:
        pal_mmio_write32(Base + DMACTL1,
                         (pal_mmio_read32(Base + DMACTL1) & PASID_TLP_STOP_MASK));
    ...
    case TXN_NO_SNOOP_ENABLE:
        /* enabling the NO SNOOP */
        pal_mmio_write32(Base + DMACTL1,
                         pal_mmio_read32(Base + DMACTL1) | NO_SNOOP_START_MASK);
        return 0;

    case TXN_NO_SNOOP_DISABLE:
        /* disabling the NO SNOOP */
        pal_mmio_write32(Base + DMACTL1,
                         pal_mmio_read32(Base + DMACTL1) & NO_SNOOP_STOP_MASK);
        return 0;

    case ATS_TXN_REQ:
        pal_mmio_write32(Base + DMA_BUS_ADDR, (uint32_t)Param);
        pal_mmio_write32(Base + ATSCTL, ATS_TRIGGER);
        return !(pal_mmio_read32(Base + ATSCTL) & ATS_STATUS);

    case START_TXN_MONITOR:
        pal_mmio_write32(Base + TXN_CTRL_BASE, TXN_START);
        return 0;

    case STOP_TXN_MONITOR:
        pal_mmio_write32(Base + TXN_CTRL_BASE, TXN_STOP);
        return 0;

    case INJECT_ERROR:
        pal_exerciser_find_pcie_capability(DVSEC, Bdf, PCIE, &CapabilityOffset);
        data = pal_mmio_read32(Ecam + pal_exerciser_get_pcie_config_offset(Bdf) +
                               CapabilityOffset + DVSEC_CTRL);
        data = data | (1 << ERROR_INJECT_BIT);
        pal_mmio_write32(Ecam + pal_exerciser_get_pcie_config_offset(Bdf) +
                         CapabilityOffset + DVSEC_CTRL, data);
                                                                                 ...
                                                                                 uint64_t Ecam)
{
    ...
    */
    ...
    /* In the Latest version of SBSA 6.0 this part of the test is obsolete hence
    * filling the reg with same data
    */
    ...
    case EXERCISER_DATA_CFG_SPACE:
        for (Index = 0; Index < TEST_REG_COUNT; Index++) {
            Data->cfg_space.reg[Index].offset =
                (uint32_t)(offset_table[Index] + pal_exerciser_get_pcie_config_offset(Bdf));
            Data->cfg_space.reg[Index].attribute = attr_table[Index];
            Data->cfg_space.reg[Index].value =
                (uint32_t)(pal_mmio_read32(EcamBase + offset_table[Index]));
        }
        return 0;
