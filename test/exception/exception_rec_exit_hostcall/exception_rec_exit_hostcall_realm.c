...
void exception_rec_exit_hostcall_realm(void)
{
    ...
    for (index = 0; index < EXCEPTION_TEST_HOSTCALL_GPRS_CHECK_COUNT; index++)
    {
        if (gGPRS[index] != lGPRS[index])
        {
            LOG(TEST, "\tHOSTCALL check, the gprs value is corrupted, \
                              hence testcase failed : line %d\n", __LINE__, 0);
            val_set_status(RESULT_FAIL(VAL_ERROR_POINT(1)));
            goto test_exit;
        }
    }
    LOG(TEST, "\tREALM HOSTCALL Trigger checks are verified \n", 0, 0);
test_exit:
