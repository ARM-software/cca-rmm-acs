...
#include "rmi_vdev_destroy_data.h"
...
static uint64_t invalid_pdev_prep_sequence(void)
{
    ...
    c_args_invalid.rd = vdev_dev.realm_rd;
    c_args_invalid.pdev = pdev_dev.pdev;
    c_args_invalid.vdev = vdev_dev.vdev;
    c_args_invalid.bdf = pdev_dev.bdf;
    c_args_invalid.doe_cap_base = pdev_dev.doe_cap_base;
...
static uint64_t vdev_state_lock_prep_sequence(void)
{
    val_smc_param_ts args;

    args = val_host_rmi_vdev_get_state(c_args_invalid.vdev);
    if (args.x0)
    {
    ...
    if (val_host_dev_communicate(&c_args_invalid.realm_invalid, &c_args_invalid.pdev_dev_invalid,
                                            &c_args_invalid.vdev_dev_invalid, RMI_VDEV_UNLOCKED))
    {
    ...
    args = val_host_rmi_vdev_lock(c_args_invalid.rd, c_args_invalid.pdev, c_args_invalid.vdev);
    if (args.x0)
    {
        LOG(ERROR, "\tVDEV lock failed, ret=%x\n", args.x0);
        goto exit;
    }

    if (val_host_dev_communicate(&c_args_invalid.realm_invalid, &c_args_invalid.pdev_dev_invalid,
                                              &c_args_invalid.vdev_dev_invalid, RMI_VDEV_LOCKED))
    {
        LOG(ERROR, "VDEV communicate failed.\n");
        goto exit;
    }
    ...
    /* Check VDEV state is RMI_VDEV_LOCKED */
    if (args.x1 != RMI_VDEV_LOCKED)
    {
        LOG(ERROR, "VDEV state should be RMI_VDEV_LOCKED, ret %lx\n", args.x1);
        goto exit;
    }

    return c_args_invalid.vdev;
...
static uint64_t intent_to_seq(struct stimulus *test_data, struct arguments *args)
{
    ...
    switch (label)
    {
        ...
        case VDEV_STATE_VDEV_LOCKED:
            args->rd = c_args_invalid.rd;
            args->pdev_ptr = c_args_invalid.pdev;
            args->vdev_ptr = vdev_state_lock_prep_sequence();
...
void cmd_vdev_destroy_host(void)
{
    uint64_t i;
    ...
    for (i = 0; i < (sizeof(test_data) / sizeof(struct stimulus)); i++)
    {
        ...
        cmd_ret = val_host_rmi_vdev_destroy(args.rd, args.pdev_ptr, args.vdev_ptr);
    ...
    cmd_ret = val_host_rmi_vdev_destroy(c_args.rd_valid, c_args.pdev_ptr_valid,
