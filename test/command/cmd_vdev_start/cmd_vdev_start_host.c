...
static uint64_t valid_input_args_prep_sequence(void)
{
    ...
    if (val_host_dev_communicate(&c_args.realm_valid, &c_args.pdev_dev_valid,
                                 &c_args.vdev_dev_valid, RMI_VDEV_UNLOCKED))
    {
    ...
    {
        LOG(ERROR, "VDEV state should be RMI_VDEV_UNLOCKED, ret %lx\n", args.x1);
        goto exit;
    }

    args = val_host_rmi_vdev_lock(c_args.rd_valid, c_args.pdev_ptr_valid,
    ...
    if (val_host_dev_communicate(&c_args.realm_valid, &c_args.pdev_dev_valid,
                                 &c_args.vdev_dev_valid, RMI_VDEV_LOCKED))
    {
        LOG(ERROR, "VDEV communicate failed\n");
        goto exit;
    }
...
void cmd_vdev_start_host(void)
{
    ...
    for (i = 0; i < (sizeof(test_data) / sizeof(struct stimulus)); i++)
    {
        ...
        cmd_ret = val_host_rmi_vdev_start(args.rd, args.pdev_ptr, args.vdev_ptr);
    ...
    cmd_ret = val_host_rmi_vdev_start(c_args.rd_valid, c_args.pdev_ptr_valid,
                                      c_args.vdev_ptr_valid);
    if (cmd_ret.x0 != 0)
    {
    ...
    if (val_host_dev_communicate(&c_args.realm_valid, &c_args.pdev_dev_valid,
                                  &c_args.vdev_dev_valid, RMI_VDEV_STARTED))
    {
        LOG(ERROR, "VDEV communicate failed\n");
    ...
    if (cmd_ret.x1 != RMI_VDEV_STARTED)
    {
        LOG(ERROR, "VDEV state should be RMI_VDEV_STARTED, ret %d\n", cmd_ret.x1);
