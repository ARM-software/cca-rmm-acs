...
static struct invalid_argument_store {
    uint64_t pdev_ready_ptr;
    val_host_pdev_ts pdev_dev_ready;
} c_args_invalid;
...
static uint64_t pdev_ready_prep_sequence(void)
{
    ...
    c_args_invalid.pdev_dev_ready = pdev_dev;
...
static uint64_t valid_input_args_prep_sequence(void)
{
    ...
    status = val_pcie_get_rootport(pdev_dev.bdf, &rp_bdf);
    if (status != 0)
        return VAL_TEST_PREP_SEQ_FAILED;
...
static uint64_t intent_to_seq(struct stimulus *test_data, struct arguments *args)
{
    ...
    switch (label)
    {
        ...
        case PDEV_STATE_PDEV_READY:
            args->pdev_ptr = pdev_ready_prep_sequence();
            if (args->pdev_ptr == VAL_TEST_PREP_SEQ_FAILED)
                return VAL_ERROR;
            c_args_invalid.pdev_ready_ptr = args->pdev_ptr;
            break;
...
void cmd_pdev_abort_host(void)
{
    ...
    for (i = 0; i < (sizeof(test_data) / sizeof(struct stimulus)); i++)
    {
        ...
        cmd_ret = val_host_rmi_pdev_abort(args.pdev_ptr);
    ...
    cmd_ret = val_host_rmi_pdev_abort(c_args.pdev_ptr_valid);
    ...
    /* PDEV state should be new as valid prep sequence pre pdev state was PDEV_NEW */
    ...
    /* If PDEV state is PDEV_COMMUNICATING post PDEV_ABORT it will change to PDEV_READY */
    cmd_ret = val_host_rmi_pdev_ide_key_refresh(c_args_invalid.pdev_ready_ptr, RMI_NCOH);
    if (cmd_ret.x0)
    {
        LOG(ERROR, "PDEV IDE Key refresh  failed, ret %x\n", cmd_ret.x0);
        val_set_status(RESULT_FAIL(VAL_ERROR_POINT(6)));
        goto destroy_device;
    }

    /* PDEV state should be PDEV_COMMUNICATING */
    if (val_host_check_pdev_state(c_args_invalid.pdev_ready_ptr, RMI_PDEV_COMMUNICATING))
    {
        LOG(ERROR, "PDEV state check failed.\n");
    ...
    cmd_ret = val_host_rmi_pdev_abort(c_args_invalid.pdev_ready_ptr);
    if (cmd_ret.x0 != RMI_SUCCESS)
    {
    ...
    if (val_host_check_pdev_state(c_args_invalid.pdev_ready_ptr, RMI_PDEV_READY))
    {
        LOG(ERROR, "PDEV state check failed.\n");
...
destroy_device:
    if (c_args_invalid.pdev_ready_ptr &&
        val_host_pdev_teardown(&c_args_invalid.pdev_dev_ready,
                               c_args_invalid.pdev_ready_ptr))
    {
