...
#include "rmi_vdev_get_measurements_data.h"
...
static uint64_t valid_input_args_prep_sequence(void)
{
    ...
    c_args.params_ptr_valid = (uint64_t)vdev_measure_params;
...
void cmd_vdev_get_measurements_host(void)
{
    uint64_t i;
    ...
    val_memset(&c_args, 0, sizeof(c_args));
    val_memset(&c_args_invalid, 0, sizeof(c_args_invalid));
    ...
    for (i = 0; i < (sizeof(test_data) / sizeof(struct stimulus)); i++)
    {
        ...
        cmd_ret = val_host_rmi_vdev_get_measurements(args.rd, args.pdev_ptr,
                                                     args.vdev_ptr, args.params_ptr);
    ...
    cmd_ret = val_host_rmi_vdev_get_measurements(c_args.rd_valid,
                                                 c_args.pdev_ptr_valid,
                                                 c_args.vdev_ptr_valid,
                                                 c_args.params_ptr_valid);
    ...
    if (val_host_dev_communicate(&c_args.realm_valid, &c_args.pdev_dev_valid,
                                 &c_args.vdev_dev_valid, RMI_VDEV_NEW))
    {
