...
#include "rmi_rtt_aux_map_unprotected_data.h"
...
#define IPA_UNPROT_UNASSIGNED   IPA_UNPROT_BASE
#define IPA_UNPROT_AUX_UNMAPPED IPA_UNPROT_BASE + L2_SIZE
...
{
    if (create_mapping(IPA_UNPROT_AUX_UNMAPPED, false, realm[VALID_REALM].rd))
    {
    ...
    return IPA_UNPROT_AUX_UNMAPPED;
}
...
static uint64_t rd_rtt_tree_single_prep_sequence(void)
{
    ...
    val_memcpy(&realm[SINGLE_RTT_TREE_REALM].flags, &realm_flags,
                                         sizeof(realm[SINGLE_RTT_TREE_REALM].flags));
...
static uint64_t intent_to_seq(struct stimulus *test_data, struct arguments *args)
{
    ...
    switch (label)
    {
        ...
        case INDEX_OOB_PRI_STATE_UNASSINGED_NS:
            args->rd = c_args.rd_valid;
            args->ipa = ipa_unprotected_unassigned_prep_sequence(c_args.rd_valid);
            if (args->ipa == VAL_TEST_PREP_SEQ_FAILED)
                return VAL_ERROR;
...
void cmd_rtt_aux_map_unprotected_host(void)
{
    uint64_t i, ret;
    ...
    for (i = 0; i < (sizeof(test_data) / sizeof(struct stimulus)); i++)
    {
        ...
        cmd_ret = val_host_rmi_rtt_aux_map_unprotected(args.rd, args.ipa, args.index);
