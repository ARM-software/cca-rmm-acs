...
#include "rmi_rtt_aux_destroy_data.h"
...
#define IPA_NO_PARENT_RTT L1_SIZE
#define IPA_NO_PARENT_RTT L1_SIZE
#define MAP_LEVEL 3
...
{
   if (val_host_create_aux_mapping(realm[VALID_REALM].rd, IPA_NON_LIVE_RTT, RTT_INDEX_1))
    {
        LOG(ERROR, "\tRTT creation failed\n", 0, 0);
        return VAL_TEST_PREP_SEQ_FAILED;
    }
...
static uint64_t intent_to_seq(struct stimulus *test_data, struct arguments *args)
{
    ...
    switch (label)
    {
        ...
        case IPA_LEVEL_UNALIGNED:
            args->rd = c_args.rd_valid;
            args->ipa = c_args.ipa_valid - PAGE_SIZE;
            args->level = c_args.level_valid;
            args->index = c_args.index_valid;
            break;
...
void cmd_rtt_aux_destroy_host(void)
{
    uint64_t i, ret;
    ...
    for (i = 0; i < (sizeof(test_data) / sizeof(struct stimulus)); i++)
    {
        ...
        cmd_ret = val_host_rmi_rtt_aux_destroy(args.rd, args.ipa, args.level, args.index);
    ...
    cmd_ret = val_host_rmi_rtt_aux_destroy(c_args.rd_valid, c_args.ipa_valid,
                                                 c_args.level_valid, c_args.index_valid);
    if (cmd_ret.x0 != 0)
    {
        LOG(ERROR, "\n\tRTT_AUX_DESTROY Command failed. %x\n", cmd_ret.x0, 0);
    ...
    /* Executing RMI_RTT_AUX_DESTROY AGAIN should fail with RMI_ERROR_RTT_AUX at L2 */
    cmd_ret = val_host_rmi_rtt_aux_destroy(c_args.rd_valid, c_args.ipa_valid,
                                                 c_args.level_valid, c_args.index_valid);
    if (RMI_STATUS(cmd_ret.x0) != RMI_ERROR_RTT_AUX || RMI_INDEX(cmd_ret.x0) != LEVEL_L2)
    {
        LOG(ERROR, "\n\tUnexpected output for RTT_AUX_DESTROY %x\n", cmd_ret.x0, 0);
