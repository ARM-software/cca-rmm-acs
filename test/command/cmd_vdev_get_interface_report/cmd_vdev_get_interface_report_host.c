...
#include "rmi_vdev_get_interface_report_data.h"
...
void cmd_vdev_get_interface_report_host(void)
{
    uint64_t i;
    ...
    for (i = 0; i < (sizeof(test_data) / sizeof(struct stimulus)); i++)
    {
        ...
        cmd_ret = val_host_rmi_vdev_get_interface_report(args.rd, args.pdev_ptr,
                                                         args.vdev_ptr);
    ...
    cmd_ret = val_host_rmi_vdev_get_interface_report(c_args.rd_valid,
                                                     c_args.pdev_ptr_valid,
                                                     c_args.vdev_ptr_valid);
    if (cmd_ret.x0 != 0)
    {
    ...
    if (cmd_ret.x1 != RMI_VDEV_LOCKED)
    {
        LOG(ERROR, "VDEV state should be RMI_VDEV_LOCKED, ret %x.\n", cmd_ret.x1);
