...
#include "rmi_rtt_aux_map_protected_data.h"
...
static uint64_t ipa_aux_rtt_unmapped_prep_sequence(void)
{
    uint64_t ret = g_data_prep_sequence(c_args.rd_valid, IPA_AUX_UNMAPPED);
    if (ret == VAL_TEST_PREP_SEQ_FAILED)
        return VAL_TEST_PREP_SEQ_FAILED;

    return IPA_AUX_UNMAPPED;
}

static uint64_t ipa_aux_state_destroyed_prep_sequence(void)
{
    uint64_t phys, rtt, ipa_base, ipa_top;
    val_data_create_ts data_create;

    if (create_mapping(IPA_AUX_DESTROYED, true, realm[VALID_REALM].rd))
    {
        LOG(ERROR, " Mapping creation failed\n");
        return VAL_TEST_PREP_SEQ_FAILED;
    }

    if (val_host_create_aux_mapping(realm[VALID_REALM].rd, IPA_AUX_DESTROYED, RTT_INDEX_1))
    {
    ...
    data_create.ipa = IPA_AUX_DESTROYED;
    ...
    if (val_host_rmi_rtt_fold(realm[VALID_REALM].rd, data_create.ipa, MAP_LEVEL, &rtt))
    {
        LOG(ERROR, "RTT_FOLD failed with ret value = %x\n");
        return VAL_TEST_PREP_SEQ_FAILED;
    }

    ipa_base = data_create.ipa;
    ipa_top = ipa_base + data_create.size;
    while (ipa_base < ipa_top)
    {
        if (val_host_rmi_rtt_aux_unmap_protected(realm[VALID_REALM].rd, ipa_base, RTT_INDEX_1).x0)
        {
            LOG(ERROR, "RTT AUX UNMAP PROTECTED failed\n");
            return VAL_TEST_PREP_SEQ_FAILED;
        }
    ...
    if (val_host_rmi_rtt_aux_destroy(realm[VALID_REALM].rd, data_create.ipa,
                                                         MAP_LEVEL, RTT_INDEX_1).x0)
    {
        LOG(ERROR, "RTT_AUX Fold failed\n");
        return VAL_TEST_PREP_SEQ_FAILED;
    }

    return IPA_AUX_DESTROYED;
}
...
{
    uint64_t test_ipa = ipa_protected_assigned_ram_prep_sequence(c_args.rd_valid);
...
static uint64_t intent_to_seq(struct stimulus *test_data, struct arguments *args)
{
    ...
    switch (label)
    {
        ...
        case IPA_UNALIGNED:
            args->rd = c_args.rd_valid - PAGE_SIZE / 2;
            args->ipa = c_args.ipa_valid;
            args->index = c_args.index_valid;
            break;
        ...
        case AUX_STATE_DESTROYED:
            args->rd = c_args.rd_valid;
            args->ipa = ipa_aux_state_destroyed_prep_sequence();
            if (args->ipa == VAL_TEST_PREP_SEQ_FAILED)
                return VAL_ERROR;
            args->index = c_args.index_valid;
            break;

        case AUX_RTT_UNMAPPED:
            args->rd = c_args.rd_valid;
            args->ipa = ipa_aux_rtt_unmapped_prep_sequence();
            if (args->ipa == VAL_TEST_PREP_SEQ_FAILED)
                return VAL_ERROR;
            args->index = c_args.index_valid;
            break;

        case INDEX_OOB_PRI_STATE_UNASSIGNED:
            args->rd = c_args.rd_valid;
            args->ipa = ipa_protected_unassigned_ram_prep_sequence(c_args.rd_valid);
            if (args->ipa == VAL_TEST_PREP_SEQ_FAILED)
                return VAL_ERROR;
...
void cmd_rtt_aux_map_protected_host(void)
{
    uint64_t i, ret;
    ...
    for (i = 0; i < (sizeof(test_data) / sizeof(struct stimulus)); i++)
    {
        ...
        cmd_ret = val_host_rmi_rtt_aux_map_protected(args.rd, args.ipa, args.index);
    ...
    if (cmd_ret.x0 != 0)
    {
        LOG(ERROR, " AUX_MAP_PROTECTED Command failed. %x\n", cmd_ret.x0);
        val_set_status(RESULT_FAIL(VAL_ERROR_POINT(4)));
        goto exit;
    }

    /* Successful execution of RMI_RTT_AUX_UNMAP_PROTECTED indicates successful transition
     * RTTE state to ASSIGNED */
    cmd_ret = val_host_rmi_rtt_aux_unmap_protected(c_args.rd_valid, c_args.ipa_valid,
                                                                     c_args.index_valid);
    if (cmd_ret.x0 != 0)
    {
        LOG(ERROR, " AUX_UNMAP_PROTECTED Command failed. %x\n", cmd_ret.x0);
...
exit:
    ...
    ret = val_host_rmi_rtt_read_entry(c_args.rd_valid, IPA_AUX_DESTROYED,
    ...
    if (rtte.walk_level != realm[VALID_REALM].s2_starting_level) {
        ...
        ret = val_host_create_rtt_levels(&realm[VALID_REALM], IPA_AUX_DESTROYED,
