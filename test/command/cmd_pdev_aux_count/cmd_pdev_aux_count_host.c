...
void cmd_pdev_aux_count_host(void)
{
    ...
    if (args.x1 > MAX_PDEV_AUX_GRANULES) {
        LOG(ERROR, "PDEV AUX count greater than %d is not valid, return aux count is: %x\n",
                                                             MAX_PDEV_AUX_GRANULES, args.x1);
