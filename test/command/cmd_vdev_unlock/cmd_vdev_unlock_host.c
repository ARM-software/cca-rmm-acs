...
#include "rmi_vdev_unlock_data.h"
...
static uint64_t valid_input_args_prep_sequence(void)
{
    ...
    args = val_host_rmi_vdev_lock(c_args.rd_valid, c_args.pdev_ptr_valid, c_args.vdev_ptr_valid);
    if (args.x0)
    {
...
void cmd_vdev_unlock_host(void)
{
    uint64_t i;
    ...
    for (i = 0; i < (sizeof(test_data) / sizeof(struct stimulus)); i++)
    {
        ...
        cmd_ret = val_host_rmi_vdev_unlock(args.rd, args.pdev_ptr, args.vdev_ptr);
    ...
    cmd_ret = val_host_rmi_vdev_unlock(c_args.rd_valid, c_args.pdev_ptr_valid,
