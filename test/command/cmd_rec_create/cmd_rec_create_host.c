...
static uint64_t params_prep_sequence(prep_seq_type type)
{
    ...
    for (i = 0; i < aux_count; i++) {
        ...
        realm_test[REALM_VALID].rec_aux_granules[i] = aux_rec;
    }
    ...
    switch (type)
    {
        ...
        case MPIDR_UPPER_INVALID:
            params->mpidr = 0xFFFFFFFF00000000;
            break;

        case MPIDR_MIDDLE_INVALID:
            params->mpidr = 0xF0;
            break;
        ...
        case AUX_GRAN_UNALINED:
             for (i = 0; i < aux_count; i++) {
                ...
                if (aux_rec == VAL_TEST_PREP_SEQ_FAILED)
                    return VAL_TEST_PREP_SEQ_FAILED;
                params->rec_aux_granules[i] = g_unaligned_prep_sequence(aux_rec);
                realm_test[REALM_VALID].rec_aux_granules[i] = aux_rec;
            }
            break;

        case INVALID_NUM_AUX:
            params->num_rec_aux = aux_count + 1;
            break;

        case PARAMS_AUX_UNDELEGATED:
            for (i = 0; i < aux_count; i++)
                params->rec_aux_granules[i] = g_undelegated_prep_sequence();
            break;

        case PARAMS_AUX_RD:
            for (i = 0; i < aux_count; i++)
                params->rec_aux_granules[i] = c_args.rd_valid;
            break;

        case PARAMS_AUX_REC:
            for (i = 0; i < aux_count; i++)
                params->rec_aux_granules[i] = c_args_invalid.rec_gran;
            break;

        case PARAMS_AUX_DATA:
            for (i = 0; i < aux_count; i++)
                params->rec_aux_granules[i] = c_args_invalid.data_gran;
            break;

        case PARAMS_AUX_RTT:
            for (i = 0; i < aux_count; i++)
                params->rec_aux_granules[i] = realm_test[REALM_VALID].rtt_l0_addr;
            break;

        case PARAMS_AUX_ALIASED:
            params->rec_aux_granules[1] = params->rec_aux_granules[0];
            break;

        case INVALID_PARAMS_FLAGS:
            params->flags = 0x02;
            break;
...
static uint64_t intent_to_seq(struct stimulus *test_data, struct arguments *args)
{
    ...
    switch (label)
    {
        ...
        case MPIDR_INVALID_UPPER:
            args->rec = c_args.rec_valid;
            args->rd = c_args.rd_valid;
            args->params_ptr = params_prep_sequence(MPIDR_UPPER_INVALID);
            if (args->params_ptr == VAL_TEST_PREP_SEQ_FAILED)
                return VAL_ERROR;
            break;

        case MPIDR_INVALID_MIDDLE:
            args->rec = c_args.rec_valid;
            args->rd = c_args.rd_valid;
            args->params_ptr = params_prep_sequence(MPIDR_MIDDLE_INVALID);
            if (args->params_ptr == VAL_TEST_PREP_SEQ_FAILED)
                return VAL_ERROR;
            break;
        ...
        case FLAGS_INVALID:
            args->rec = c_args.rec_valid;
            args->rd = c_args.rd_valid;
            args->params_ptr = params_prep_sequence(INVALID_PARAMS_FLAGS);
            if (args->params_ptr == VAL_TEST_PREP_SEQ_FAILED)
                return VAL_ERROR;
            break;
...
void cmd_rec_create_host(void)
{
    ...
    for (i = 0; i < (sizeof(test_data) / sizeof(struct stimulus)); i++)
    {
        ...
        ret = val_host_rmi_rec_create(args.rec, args.rd,
                                    args.params_ptr);
    ...
    ret = val_host_rmi_rec_create(c_args.rec_valid, c_args.rd_valid,
                                  c_args.params_valid);
