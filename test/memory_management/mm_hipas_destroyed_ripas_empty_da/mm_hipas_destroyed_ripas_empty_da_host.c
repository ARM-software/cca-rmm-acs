...
void mm_hipas_destroyed_ripas_empty_da_host(void)
{
    ...
    ret = val_host_rmi_rtt_destroy(realm.rtt_l3[(realm.rtt_l3_count - 1)].rtt_addr,
                                realm.rd, data_create.ipa, VAL_RTT_MAX_LEVEL);
    if (ret)
    {
        LOG(ERROR, "\trealm_rtt_destroy failed, rtt=0x%x, ret=0x%x\n",
                                realm.rtt_l3[(realm.rtt_l3_count - 1)].rtt_addr, ret);
        val_set_status(RESULT_FAIL(VAL_ERROR_POINT(12)));
        goto destroy_realm;
    }

   /* Test intent: Protected IPA, HIPAS=DESTROYED, RIPAS=EMPTY data access
    ...
    if (validate_rec_exit_da(rec_exit, ripas_ipa, ESR_ISS_DFSC_TTF_L2))
    {
