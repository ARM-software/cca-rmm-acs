...
void mm_rtt_fold_destroyed_host(void)
{
    ...
    ipa = IPA_ALIGNED_2MB;
    size = data_create.size;
    while (size < ALIGNED_2MB)
    {
        ret = val_host_rmi_data_destroy(realm.rd, ipa);
        if (ret)
        {
            LOG(ERROR, "\tval_rmi_data_destroy failed, ret=0x%x ipa %lx\n", ret, ipa);
