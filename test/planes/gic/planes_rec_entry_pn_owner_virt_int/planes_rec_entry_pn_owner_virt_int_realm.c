...
static void p0_payload(void)
{
    ...
    plane_flags.gic_owner = RSI_GIC_OWNER_N;
    ...
    if (run_ptr.exit.gprs[0] != PSI_P0_CALL)
    {
        LOG(ERROR, "Unexpected REC entry path\n", 0, 0);
        val_set_status(RESULT_FAIL(VAL_ERROR_POINT(6)));
        goto exit;
    }
...
void planes_rec_entry_pn_owner_virt_int_realm(void)
{
    if (val_realm_in_p0())
        p0_payload();
    else
