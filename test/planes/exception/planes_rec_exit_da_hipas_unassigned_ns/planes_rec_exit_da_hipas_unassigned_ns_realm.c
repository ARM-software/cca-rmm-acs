...
static void p0_payload(void)
{
    ...
    /* Request Host the test IPA whose HIPAS is UNASSIGNED_NS via host call*/
...
static void p1_payload(void)
{
    ...
    /* Test Intent: Unprotected IPA, HIPAS = UNASSIGNED_NS
...
void planes_rec_exit_da_hipas_unassigned_ns_realm(void)
{
    if (val_realm_in_p0())
        p0_payload();
    else
