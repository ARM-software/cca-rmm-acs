...
void planes_rec_exit_host_call_host(void)
{
    ...
    if (rec_exit->exit_reason != RMI_EXIT_HOST_CALL || rec_exit->plane != PLANE_1_INDEX) {
        LOG(ERROR, "\tUnexpected REC exit, %d. Plane index: %lx \n",
                                     rec_exit->exit_reason, rec_exit->plane);
