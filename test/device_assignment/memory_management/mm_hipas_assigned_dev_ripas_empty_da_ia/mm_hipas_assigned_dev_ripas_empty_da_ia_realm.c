...
static uint64_t validate_digests(val_realm_rsi_vdev_info_ts *rsi_device_info)
{
    ...
    val_realm_verify_digests_ts verify_digests[] = {
        ...
        //     shared_pubkey_buff,
        //     pubkey_buff_size,
        //     (void *)rsi_device_info->pubkey_digest
        // },
        {
            shared_report_buff,
            report_buff_size,
    ...
    for (i = 0; i < 4 ; i++)
    {
        ...
        {
            LOG(ERROR, "Digests are not matching.\n");
            return VAL_ERROR;
        }
    }
    return VAL_SUCCESS;
...
void mm_hipas_assigned_dev_ripas_empty_da_ia_realm(void)
{
    ...
    for (i = 0; i < interface_report.mmio_range_count; i++)
    {
        ...
        if ((args.x0 != RSI_SUCCESS) || (args.x2 != RSI_EMPTY) || (args.x1 != ipa_top)) {
   ...
   /* Test intent: RIPAS=Empty and HIPAS=ASSIGNED_DEV, data access
    * => SEA to Realm
    */
    LOG(TEST, "RIPAS=Empty and HIPAS=ASSIGNED_DEV, data access => SEA to Realm\n");
    ...
    g_sea_params.far = mmio_range_struct[0].first_page;
    *(volatile uint32_t *)mmio_range_struct[0].first_page = 0x100;
    dsbsy();
   ...
   /* Test intent: RIPAS=Empty and HIPAS=ASSIGNED_DEV, instruction fetch
    * => SEA to Realm
    */
    LOG(TEST, "RIPAS=Empty and HIPAS=ASSIGNED_DEV, instruction fetch => SEA to Realm\n");
    val_memset(&g_sea_params, 0, sizeof(g_sea_params));
    g_sea_params.abort_type = EC_INSTRUCTION_ABORT_SAME_EL;
    g_sea_params.far = mmio_range_struct[0].first_page;
    fun_ptr = (void *)mmio_range_struct[0].first_page;
    (*fun_ptr)();
