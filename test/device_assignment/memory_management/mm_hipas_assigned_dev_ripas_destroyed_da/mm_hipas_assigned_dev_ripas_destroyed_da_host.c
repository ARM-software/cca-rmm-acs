...
void mm_hipas_assigned_dev_ripas_destroyed_da_host(void)
{
    ...
    do
    {
        ...
        ipa_base = rec_exit->dev_mem_base;
    ...
    args = val_host_rmi_vdev_unmap(realm.rd, ipa_base, VAL_RTT_MAX_LEVEL);
    if (args.x0)
    {
        LOG(ERROR, "\trmi_vdev_unmap failed: %lx\n", args.x0);
        val_set_status(RESULT_FAIL(VAL_ERROR_POINT(63)));
        goto destroy_vdev;
    }

    ret = val_host_rmi_granule_undelegate(ipa_base);
    if (ret)
    {
        LOG(ERROR, "\trmi_granule_undelegate failed: %lx\n", ret);
        val_set_status(RESULT_FAIL(VAL_ERROR_POINT(64)));
        goto destroy_vdev;
    }
   ret = val_host_dev_mem_map(&realm, vdev, ipa_base, (ipa_base + PAGE_SIZE), ipa_base);
    if (ret)
    {
        LOG(ERROR, "\tval_host_dev_mem_map failed \n");
    ...
    ret = val_host_rmi_rtt_read_entry(realm.rd, ipa_base, VAL_RTT_MAX_LEVEL, &rtte);
    if (ret)
    {
    ...
    if ((rtte.state != RMI_ASSIGNED_DEV) || (rtte.ripas != RSI_DESTROYED))
    {
    ...
    {
        LOG(ERROR, "\tRec enter failed, ret=%x\n", ret);
        val_set_status(RESULT_FAIL(VAL_ERROR_POINT(68)));
        goto destroy_vdev;
    }
    ...
    {
        LOG(ERROR, "\tRec enter failed, ret=%x\n", ret);
        val_set_status(RESULT_FAIL(VAL_ERROR_POINT(71)));
        goto destroy_vdev;
    }

    rec_enter->gprs[1] = ipa_base;
    ret = val_host_rmi_rec_enter(realm.rec[0], realm.run[0]);
    if (ret)
    {
        LOG(ERROR, "\tRec enter failed, ret=%x\n", ret);
        val_set_status(RESULT_FAIL(VAL_ERROR_POINT(72)));
        goto destroy_vdev;
    }

    if (validate_rec_exit_da(rec_exit, ipa_base, ESR_ISS_DFSC_TTF_L3,
                                NON_EMULATABLE_DA, 0))
    {
        LOG(ERROR, "REC exit params mismatch\n");
        val_set_status(RESULT_FAIL(VAL_ERROR_POINT(73)));
        goto destroy_vdev;
    }
    ...
    {
        LOG(ERROR, "\trmi_vdev_unmap failed: %lx\n", args.x0);
        val_set_status(RESULT_FAIL(VAL_ERROR_POINT(74)));
        goto destroy_vdev;
    }
    ...
    {
        LOG(ERROR, "\trmi_granule_undelegate failed: %lx\n", ret);
        val_set_status(RESULT_FAIL(VAL_ERROR_POINT(75)));
        goto destroy_vdev;
    }
