...
void rhi_da_vdev_p2p_unbind_host(void)
{
    ...
    ret = da_init(&realm, &pdev_obj, &vdev_obj[0], RMI_VDEV_LOCKED);
    if (ret)
    {
    ...
    vdev_obj[0].vdev_id = 0;
    ret = da_create_vdev(&realm, &pdev_obj, &vdev_obj[1], 1, RMI_VDEV_LOCKED);
    if (ret)
    {
        LOG(ERROR, "Second VDEV init failed, ret=%lx\n", ret);
    ...
    rec_enter->gprs[2] = vdev_obj[1].vdev_id;
...
destroy_vdev2:
    if (val_host_vdev_teardown(&realm, &pdev_obj, &vdev_obj[1]))
    {
...
destroy_vdev1:
    if (val_host_vdev_teardown(&realm, &pdev_obj, &vdev_obj[0]))
    {
