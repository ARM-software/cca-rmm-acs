...
static uint64_t rhi_da_vdev_get_measurements(uint64_t vdev_id, uint64_t params_ipa,
                                             uint64_t expected, uint64_t *handle)
{
    __attribute__((aligned (PAGE_SIZE))) val_realm_rsi_host_call_t rhi_call = {0};
    uint64_t ret;
    ...
    if ((rhi_call.gprs[0] == RHI_DA_ERROR_INCOMPLETE) && handle) {
        *handle = rhi_call.gprs[1];
        if (*handle == 0) {
            LOG(ERROR, "RHI_DA_VDEV_GET_MEASUREMENTS returned zero handle\n");
...
void rhi_da_vdev_get_measurements_realm(void)
{
    uint64_t ret;
    val_realm_rsi_host_call_t *gv_realm_host_call;
    val_realm_rsi_dev_measure_params_ts *params;
    uint64_t vdev_id;
    uint64_t handle = 0;
    uint64_t i;
    ...
    params = (val_realm_rsi_dev_measure_params_ts *)(val_get_shared_region_base() +
                                                    TEST_USE_OFFSET1);
    val_memset(params, 0, sizeof(*params));
    params->indices[31] = (unsigned char)1U << 6U;

    const struct meas_step steps[] = {
        {"invalid vdev id", INVALID_VDEV_ID, (uint64_t)params,
            RHI_DA_ERROR_INVALID_VDEV_ID, false},
        {"invalid params IPA", vdev_id, INVALID_PARAMS_IPA,
            RHI_DA_ERROR_ACCESS_FAILED, false},
        {"valid request", vdev_id, (uint64_t)params,
            RHI_DA_ERROR_INCOMPLETE, true},
    };

    for (i = 0; i < (sizeof(steps) / sizeof(steps[0])); i++)
    {
        LOG(TEST, "Validate RHI_DA_VDEV_GET_MEASUREMENTS %s:\n", steps[i].desc);
        ret = rhi_da_vdev_get_measurements(steps[i].vdev_id, steps[i].params_ipa,
                                           steps[i].expected,
    ...
    LOG(TEST, "RHI_DA_VDEV_GET_MEASUREMENTS completed\n");
